...
public:
    explicit MediaKeySystemFactoryProxy(const sptr<IRemoteObject> &remote)
        : IProxyBroker<OHOS::HDI::Drm::V1_0::IMediaKeySystemFactory>(remote)
    {}
    ...
    int32_t GetMediaKeySystemName(std::string &name, std::string &uuid) override;
    ...
    static int32_t GetMediaKeySystemName(std::string &name, std::string &uuid, const sptr<IRemoteObject> remote);
