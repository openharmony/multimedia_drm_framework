...
MediaKeySessionImpl::MediaKeySessionImpl()
{
    ...
    ObjectRefMap<MediaKeySessionImpl>::Insert(this);
...
MediaKeySessionImpl::~MediaKeySessionImpl()
{
    ...
    ObjectRefMap<MediaKeySessionImpl>::DecreaseRef(this);
}
    ...
    optional_view<array<OptionsData>> options)
{
    ...
    OHOS::DrmStandard::IMediaKeySessionService::MediaKeyRequestInfo mediaKeyRequestInfo;
    OHOS::DrmStandard::IMediaKeySessionService::MediaKeyRequest mediaKeyRequest;
    ...
    mediaKeyRequestInfo.mediaKeyType = (OHOS::DrmStandard::IMediaKeySessionService::MediaKeyType)mediaKeyType;
    ...
    auto *keySessionTaihe = objectGuard.GetPtr();
    if (!CheckMediaKeySessionStatus(keySessionTaihe)) {
        return getEmptyMediaKeyRequest();
    }
    int intValue = keySessionTaihe->keySessionImpl_->GenerateMediaKeyRequest(mediaKeyRequestInfo, mediaKeyRequest);
...
array<uint8_t> MediaKeySessionImpl::ProcessMediaKeyResponseSync(array_view<uint8_t> response)
{
    ...
    int32_t intValue = keySessionTaihe->keySessionImpl_->ProcessMediaKeyResponse(licenseId, resPonse);
...
array<uint8_t> MediaKeySessionImpl::GenerateOfflineReleaseRequestSync(array_view<uint8_t> mediaKeyId)
{
    ...
    int32_t intValue = keySessionTaihe->keySessionImpl_->GenerateOfflineReleaseRequest(releaseResponseLicenseId,
        releaseRequest);
    if (intValue != OHOS::DrmStandard::DRM_TAIHE_ERR_OK) {
        set_business_error(OHOS::DrmStandard::DRM_TAIHE_ERR_SERVICE_FATAL,
            "service error.");
    ...
    array_view<uint8_t> response)
{
    ...
    int32_t intValue = keySessionTaihe->keySessionImpl_->ProcessOfflineReleaseResponse(releaseResponseLicenseId,
        releaseResponse);
    if (intValue != OHOS::DrmStandard::DRM_TAIHE_ERR_OK) {
        set_business_error(OHOS::DrmStandard::DRM_TAIHE_ERR_SERVICE_FATAL,
            "service error.");
...
void MediaKeySessionImpl::RestoreOfflineMediaKeysSync(array_view<uint8_t> mediaKeyId)
{
    ...
    intValue = keySessionTaihe->keySessionImpl_->RestoreOfflineMediaKeys(restoreLicenseId);
...
ContentProtectionLevel MediaKeySessionImpl::GetContentProtectionLevel()
{
    ...
    (OHOS::DrmStandard::IMediaKeySessionService::ContentProtectionLevel)0;
    ContentProtectionLevel::key_t levelKey;
    DrmTaiheUtils::GetEnumKeyByValue<ContentProtectionLevel>(level, levelKey);
    if (keySessionImpl_ != nullptr) {
