...
#include "mediakeysessionndk_fuzzer.h"
    ...
    uint8_t *info, int32_t infoLen, char *extra)
{
    ...
    DRM_INFO_LOG("Event: the mediaKeySession object is: 0x%{public}06" PRIXPTR, FAKE_POINTER(mediaKeySessoin));
    ...
    bool hasNewGoodKeys)
{
    ...
    DRM_INFO_LOG("KeyChangedEvent: the mediaKeySession object is: 0x%{public}06" PRIXPTR,
        FAKE_POINTER(mediaKeySessoin));
...
MediaKeysessionNdkFuzzer::MediaKeysessionNdkFuzzer() {}

MediaKeysessionNdkFuzzer::~MediaKeysessionNdkFuzzer() {}

void MediaKeysessionNdkFuzzer::GetUuid()
{
...
void MediaKeysessionNdkFuzzer::IsWisePlay()
{
...
void MediaKeysessionNdkFuzzer::GenerateDeviceCertificate()
{
    DRM_INFO_LOG("MediaKeysessionNdkFuzzer::GenerateDeviceCertificate start");
...
void MediaKeysessionNdkFuzzer::GenerateLicense()
{
...
void MediaKeysessionNdkFuzzer::Init()
{
    DRM_INFO_LOG("MediaKeysessionNdkFuzzer::Init start");
    GetUuid();
    IsWisePlay();
...
void MediaKeysessionNdkFuzzer::Deinitialize()
{
    DRM_INFO_LOG("MediaKeysessionNdkFuzzer::Deinitialize start");
...
bool MediaKeysessionNdkFuzzer::FuzzTestMediaKeySessionGenerateLicenseNdk(uint8_t *rawData, size_t size)
{
    ...
    DRM_MediaKeyRequest mediaKeyRequest;
    DRM_MediaKeyRequestInfo info;
    int err = memcpy_s(&info, size, rawData, size);
    if (err != 0) {
...
bool MediaKeysessionNdkFuzzer::FuzzTestMediaKeySessionLicenseOperationNdk(uint8_t *rawData, size_t size)
{
    if (rawData == nullptr || size < sizeof(int32_t)) {
        return false;
    }
    ...
    OH_MediaKeySession_CheckMediaKeyStatus(nullptr, &mediaKeyStatus);
    OH_MediaKeySession_CheckMediaKeyStatus(mediaKeySession, &mediaKeyStatus);
    OH_MediaKeySession_ClearMediaKeys(nullptr);
    OH_MediaKeySession_ClearMediaKeys(mediaKeySession);
    OH_MediaKeySession_CheckMediaKeyStatus(mediaKeySession, &mediaKeyStatus);
...
bool MediaKeysessionNdkFuzzer::FuzzTestMediaKeySessionsSetUpLicenseNdk(uint8_t *rawData, size_t size)
{
    if (rawData == nullptr || size < sizeof(int32_t)) {
        return false;
    }
...
bool FuzzMediaKeysessionNdk(uint8_t *data, size_t size)
{
    ...
    MediaKeysessionNdkFuzzer testMediasession;

    testMediasession.FuzzTestMediaKeySessionGenerateLicenseNdk(data, size);
    testMediasession.FuzzTestMediaKeySessionLicenseOperationNdk(data, size);
    testMediasession.FuzzTestMediaKeySessionsSetUpLicenseNdk(data, size);
    return true;
}
} // namesapce OHOS
