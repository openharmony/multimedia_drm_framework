...
#define FUZZ_PROJECT_NAME "mediakeysessionndk_fuzzer"

namespace OHOS {
namespace DrmStandard {
class MediaKeysessionNdkFuzzer : public NoCopyable {
private:
    bool wisePlay = false;

public:
    MediaKeysessionNdkFuzzer();
    ~MediaKeysessionNdkFuzzer();
    bool FuzzTestMediaKeySessionGenerateLicenseNdk(uint8_t *rawData, size_t size);
    bool FuzzTestMediaKeySessionLicenseOperationNdk(uint8_t *rawData, size_t size);
    bool FuzzTestMediaKeySessionsSetUpLicenseNdk(uint8_t *rawData, size_t size);
    void GetUuid();
    void Init();
    void Deinitialize();
    void IsWisePlay();
