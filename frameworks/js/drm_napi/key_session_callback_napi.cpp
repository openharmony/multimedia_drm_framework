...
void MediaKeySessionCallbackNapi::OnMediaKeySessionKeyExpired(const std::string eventType, const KeyStatus status)
{
    DRM_INFO_LOG("MediaKeySessionCallbackNapi OnMediaKeySessionKeyExpired");
    napi_value result;
    napi_value jsCallback = nullptr;
    napi_value retVal;
    napi_status state;

    DRM_CHECK_AND_RETURN_LOG(callbackMap_.find(eventType) != callbackMap_.end(),
        "Napi OnMediaKeySessionKeyExpired, not register this callback");
    sptr<CallBackPair> item = callbackMap_[eventType];

    DRM_CHECK_AND_RETURN_LOG(item != nullptr, "Napi OnMediaKeySessionKeyExpired, sptr callbackPair is nullptr");
    ...
    DRM_INFO_LOG("Napi OnMediaKeySessionKeyExpired, Status: %{public}d", keyStatus);

    napi_get_reference_value(env, callbackRef, &jsCallback);
    state = napi_call_function(env, nullptr, jsCallback, ARGS_ONE, args, &retVal);
    DRM_CHECK_AND_RETURN_LOG(state == napi_ok, "%{public}s failed to napi_call_function", eventType.c_str());
}

void MediaKeySessionCallbackNapi::OnMediaKeySessionReclaimed(const std::string eventType, const SessionStatus status)
{
    DRM_INFO_LOG("MediaKeySessionCallbackNapi OnMediaKeySessionReclaimed");
    napi_value result;
    napi_value jsCallback = nullptr;
    napi_value retVal;
    napi_status state;
    ...
    DRM_CHECK_AND_RETURN_LOG(callbackMap_.find(eventType) != callbackMap_.end(),
        "Napi OnMediaKeySessionReclaimed, not register this callback");
    sptr<CallBackPair> item = callbackMap_[eventType];

    DRM_CHECK_AND_RETURN_LOG(item != nullptr, "Napi OnMediaKeySessionReclaimed, sptr callbackPair is nullptr");
    napi_env env = item->GetEnv();
    napi_ref callbackRef = item->GetCallback();
    ...
    DRM_INFO_LOG("Napi OnMediaKeySessionReclaimed, Status: %{public}d", sessionStatus);
